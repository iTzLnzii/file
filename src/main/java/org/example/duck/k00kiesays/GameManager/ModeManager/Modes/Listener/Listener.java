...
public class Listener implements org.bukkit.event.Listener
{
    ...
    public void onCakeEat(PlayerInteractEvent e)
    {
        ...
        if (Objects.requireNonNull(e.getClickedBlock()).getType() == Material.CAKE)
        {
    ...
    @EventHandler
    public void Milk(PlayerInteractAtEntityEvent e)
    {
        if (!plugin.getConfig().getString("current-mode").equalsIgnoreCase("milk")) return;
        List<Player> players = GetEventPlayers.getEventPlayers();
        if (players.contains(e.getPlayer()) && e.getRightClicked().getName().equalsIgnoreCase("Animal"))
        {
            Reward.give(e.getPlayer());
            e.getPlayer().getInventory().clear();
            e.getPlayer().teleport(plugin.getConfig().getLocation("winning-location"));
