...
public final class WorldEditHook {
    ...
    public static void save(Location primary, Location secondary, File schematicFile) {
        ...
        Region region = null;
        try {
            region = new CuboidRegion(BukkitAdapter.asBlockVector(primary), BukkitAdapter.asBlockVector(secondary));
        } catch (Exception e) {
        ...
        try (Closer closer = Closer.create()) {
            ...
            ClipboardWriter writer = closer.register(BuiltInClipboardFormat.SPONGE_V2_SCHEMATIC.getWriter(outputStream));
