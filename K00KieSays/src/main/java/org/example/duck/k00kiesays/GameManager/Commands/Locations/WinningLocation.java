...
public class WinningLocation implements CommandExecutor
{
    private final K00KieSays plugin;
    public WinningLocation(K00KieSays plugin)
    {
        this.plugin = plugin;
    }
    @Override
    public boolean onCommand(CommandSender sender, Command command, String Label, String[] args)
    {
        ...
        p.sendMessage(ChatColor.translateAlternateColorCodes('&',"&3[K00Kie Says]&a Winning spawn is set to: X "
                + Double.toString(loc.getX()) + " Y " + Double.toString(loc.getY()) + " Z " +Double.toString(loc.getZ()) +
                " in world " + loc.getWorld().getName()));
        plugin.getConfig().set("winning-location", loc);
        plugin.saveConfig();
        return true;
    }
}
